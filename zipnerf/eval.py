...
def main(unused_argv):
    ...
    while True:
        ...
        for idx, batch in enumerate(dataloader):
            logging.info(dataset.imageNames[idx])
            ...
            if config.eval_save_output and (config.eval_render_interval > 0):
                if (idx % config.eval_render_interval) == 0:
                    utils.save_img_u8(postprocess_fn(rendering['rgb']),
                                      path_fn(f'color_{dataset.imageNames[idx][3:]}.png'))
                    utils.save_img_u8(postprocess_fn(np.abs(rgb_gt-rendering['rgb'])),
                                      path_fn(f'color_diff_{dataset.imageNames[idx][3:]}.png'))
