...
class Config:
    ...
    compute_mask_loss: bool = True  # If True, compute mask loss.
    compute_color_loss: bool = True  # If True, compute mask loss.
    compute_depth_tvl_reg: bool = False  # If True, compute depth TVL1 reg.
    ...
    checkpoint_every: int = 2000  # The number of steps to save a checkpoint.
    ...
    data_coarse_loss_mult: float = 0.1  # Multiplier for the coarser data terms.
...
def load_config():
    ...
    if 1: #config.meta_exp == '':
        ...
        config.exp_name = str(config.meta_exp.parents[1].name +'/'+ config.meta_exp.parents[0].name +'/'+ config.meta_exp.name)
        config.meta_exp = str(config.meta_exp)
    # else:
    #     config.meta_exp = None
    #     config.exp_name = "test"
    print(config.meta_exp)
    return config
