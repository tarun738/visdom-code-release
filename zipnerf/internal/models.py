...
class Model(nn.Module):
    ...
    num_prop_samples: int = 24  # The number of samples for each proposal level.
    num_nerf_samples: int = 64  # The number of samples the final nerf level.
    ...
    """
        ...
        for i_level in range(self.num_levels):
            ...
            # print(sdist.shape,'before dilation')
            ...
            # print(sdist.shape,'before annealing')
            ...
            # print(sdist.shape,'before interval sampling')
            ...
            if 0 & self.config.vaxnerf:
                z_masks = (tdist[...,1:] >= batch['nears_vh']) & (tdist[...,1:] <= batch['nears_vh'])
                ray_results['density'].squeeze(-1)[~z_masks] = 0
                ray_results['rgb'].squeeze(-1)[~z_masks,:] = 0
