...
class TSDF:
    ...
    def integrate_tsdf(
        ...
        sampled_depth = F.grid_sample(
            input=depth_images.cuda(), grid=grid.cuda(), mode="nearest", padding_mode="zeros", align_corners=False
        )  # [batch, N, 1]
        ...
        if color_images is not None:
            sampled_colors = F.grid_sample(
                input=color_images.cuda(), grid=grid.cuda(), mode="nearest", padding_mode="zeros", align_corners=False
            )  # [batch, N, 3]
...
def main(unused_argv):
    ...
    dataloader = dataset
    # torch.utils.data.DataLoader(np.arange(len(dataset)),
    #                                          shuffle=False,
    #                                          batch_size=1,
    ...
    for i in tqdm(range(0, len(c2w), batch_size), disable=not accelerator.is_main_process):
        tsdf.integrate_tsdf(
            c2w[i: i + batch_size],
            K[i: i + batch_size].squeeze(),
            depth_images[i: i + batch_size][0].unsqueeze(0),
            color_images=color_images[i: i + batch_size][0].unsqueeze(0),
        )
