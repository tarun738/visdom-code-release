...
def get_visual_hull(N, bbox, scene_info, cam_center, usevisibledomain=False):
    ...
    for h_id in trange(N):
        ...
        if not usevisibledomain:
            pcs.append((valid_pt_mask_final.float().sum(0) >= (images.shape[0] - 1))) # [100, 100]
        else:
    ...
    if usevisibledomain:
        idx = torch.where((pcs == pcsvd) & (pcsvd>= (images.shape[0] - 1)))
    else:
...
if __name__=="__main__":
    ...
    pcd, bbox = get_visual_hull(args.voxel_num, init_bbox, scene_info, cam_center, usevisibledomain=args.vd)
    ...
    pcd, bbox_new = get_visual_hull(64, [enlarged_bbox_min, enlarged_bbox_max], scene_info, [0,0,0], usevisibledomain=args.vd)
    # save the pointcloud
    if args.sparse_id >= 0:
        if args.vd:
            o3d.io.write_point_cloud(os.path.join(args.data_dir, f"visual_hull_{str(args.sparse_id)}_vd.ply"), pcd)
        else:
