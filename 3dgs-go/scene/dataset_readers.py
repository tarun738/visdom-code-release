...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, extra_opts=None):
    ...
    # res_scale tracks how much smaller the loaded images are than the COLMAP-calibrated
    # (full) resolution, so pixel-space intrinsics (focal length, principal point) can be
    # rescaled to match: intr.width/height and intr.params are always in the original,
    # uncropped calibration resolution regardless of which images_* folder we load from.
    res_scale = 1
    if extra_opts.resolution in [1, 2, 4, 8]:
        ...
        if not osp.exists(tmp_images_folder):
            ...
            res_scale = extra_opts.resolution
    else:
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        # FoVx/FoVy above are computed from the unscaled (calibration-resolution) focal
        # length and width/height, so they're already resolution-independent. FX/FY/CX/CY
        # are stored as absolute pixel coordinates paired against the *loaded* image
        # (see Camera/Render_Camera in scene/cameras.py), so those need the same rescale.
        focal_length_x /= res_scale
        focal_length_y /= res_scale
        Cx /= res_scale
        Cy /= res_scale
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, CX = Cx, CY=Cy, FX = focal_length_x, FY=focal_length_y, image=image,
