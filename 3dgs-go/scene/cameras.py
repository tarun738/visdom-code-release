...
import numpy as np
from utils.graphics_utils import getWorld2View2, getProjectionMatrix, getProjectionMatrixShift, getWorld2View2_tensor, fov2focal
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy,  FX, FY, CX, CY,  image, gt_alpha_mask, mono_depth,
        ...
        self.projection_matrix = getProjectionMatrixShift(znear=self.znear, zfar=self.zfar, fovX=self.FoVx, fovY=self.FoVy, focal_x = self.FX, focal_y= self.FY, cx=self.CX, cy=self.CY, width=self.image_width, height=self.image_height).transpose(0,1).cuda().float()
...
class Render_Camera(nn.Module):
    def __init__(self, R, T, FoVx, FoVy, image, CX=None, CY=None, gt_alpha_mask=None, mono_depth=None,
        ...
        self.CX=self.image_width/2 if CX is None else CX
        self.CY=self.image_height/2 if CY is None else CY
