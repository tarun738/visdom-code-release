...
def training(args, dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    interpCams = args.interpCams

    first_iter = 0
    tb_writer = prepare_output_and_logger(dataset)
    gaussians = GaussianModel(dataset.sh_degree)
    scene = Scene(dataset, gaussians, extra_opts=args, interpCams=interpCams, load_best=args.load_best)
    ...
    viewpoint_stack, augview_stack, interp_stack = None, None, None
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if interpCams:
            if not interp_stack:
                interp_stack = scene.getInterpCameras().copy()
        viewpoint_cam = viewpoint_stack.pop(randint(0, len(viewpoint_stack)-1))
        if interpCams:
            interp_cam = interp_stack.pop(randint(0, len(interp_stack)-1))
        ...
        with torch.no_grad():
            ...
            # Log and save
            testpsnr = training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background))
            if (iteration in saving_iterations) or (testpsnr > besttestpsnr):
                print("\n[ITER {}] Saving Gaussians".format(iteration))
                if testpsnr > besttestpsnr:
                    scene.save('best')
                    besttestpsnr = testpsnr
...
def cal_loss(opt, args, image, render_pkg, viewpoint_cam, bg, silhouette_loss_type="bce", mono_loss_type="mid", tb_writer: Optional[SummaryWriter]=None, iteration=0):
    ...
    Ll1 = l1_loss(image, gt_image) if not regpose else torch.tensor(0.0, device=image.device)
    Lssim = (1.0 - ssim(image, gt_image)) if not regpose else torch.tensor(0.0, device=image.device)
    ...
    if hasattr(args, "use_mask") and args.use_mask:
        if silhouette_loss_type == "bce":
            if not regpose:
                ...
                silhouette_loss = -((1-viewpoint_cam.mask)*torch.log(1 - render_pkg["rendered_alpha"]+1e-5)).mean()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--load_best', action='store_true', default=False, help='use the best model')
    parser.add_argument('--interpCams', action='store_true', default=False, help='use interpCams')
