...
class GaussianDreamer(BaseLift3DSystem):
    ...
    def configure(self) -> None:
        ...
        self.load_best = self.cfg.load_best
        self.point_cloud = self.init_pointcloud(self.init_dreamer, self.load_best)
    ...
    def render_gs(self, batch: Dict[str, Any], renderbackground=None, need_loss=False) -> Dict[str, Any]:
        ...
        for id in range(batch['index'].shape[0]):
            viewpoint_cam = Render_Camera(batch['R'][id],
                                          batch['T'][id],
                                          batch['fovx'][id],
                                          batch['fovy'][id],
                                          batch['image'][id],
    ...
    def init_pointcloud(self, path, load_best=False):
        if path == 'random':
            pcb = self.pcb()
        ...
        if load_best:
            ply_path = os.path.join(path, 'point_cloud', 'iteration_best', 'point_cloud.ply')
            threestudio.info(f'init ply file from best')
        else:
            max_num = 0
            ply_path = ''
